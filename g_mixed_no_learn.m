function [gx] = g_mixed_no_learn(~,Phi,u_t,inG)
...
[gx(1),~,~] = sigm((u_t(2)*(Xt(3)) + lambda1),[],beta1); %% prediction of feedback ratings as a function of 
...
[gx(1),~,~] = sigm((u_t(2)*(Xt(3)) + lambda1),[],beta1);

% exp_vigor = eta*Xt(2);
end
else
if (cs_current && congruent_current) || (~cs_current && ~congruent_current) % trials where they received infusions
[gx(1),~,~] = sigm((u_t(2) + lambda1 + epsilon),[],beta1); %% prediction of feedback ratings as a function of 
... infusion expectancy (Xt) weighted by expectancy sensitivity eta + actual neurofeedback presented (u_t(2)) + bias lambda
    

% exp_vigor = eta*Xt(1);
elseif (~cs_current && congruent_current) || (cs_current && ~congruent_current) % trials with no infusions
[gx(1),~,~] = sigm((u_t(2) + lambda1),[],beta1);
...
if cs_current
[gx(2),~,~] = sigm((lambda2 + epsilon),[],beta2); %% prediction of infusion/no infusion expectancy ratings as a function of expectancy (Xt)
else
[gx(2),~,~] = sigm((lambda2),[],beta2);
end
