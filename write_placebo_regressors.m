...
for i = 1:length(subjs)
    ...
    try
        ...
        write3Ddeconv_startTimes(data_dump_str,s.subjects.(s.id).feedback_onset_all_runs,s.subjects.(s.id).feedback_onset_all_runs+.5,sprintf('%s_feedback',s.id),true(s.n_t,1),0,s); %Stick no duration
        write3Ddeconv_startTimes(data_dump_str,s.subjects.(s.id).feedback_onset_all_runs,s.subjects.(s.id).feedback_onset_all_runs+.5,sprintf('%s_Valence',s.id),true(s.n_t,1),0,s); %Positive=1 Negative=-1
        write3Ddeconv_startTimes(data_dump_str,s.subjects.(s.id).feedback_onset_all_runs,s.subjects.(s.id).feedback_onset_all_runs+.5,sprintf('%s_Magnitude',s.id),true(s.n_t,1),0,s); %High Feedback=1 Low Feedback=-1
        write3Ddeconv_startTimes(data_dump_str,s.subjects.(s.id).feedback_onset_all_runs,s.subjects.(s.id).feedback_onset_all_runs+.5,sprintf('%s_Congruent',s.id),true(s.n_t,1),0,s); %Congruent=1 Not Congruent=-1
        ...
        write3Ddeconv_startTimes(data_dump_str,s.subjects.(s.id).infusion_onset_all_runs,s.subjects.(s.id).infusion_onset_all_runs+.5,sprintf('%s_infusion',s.id),true(s.n_t,1),0,s);
        ...
        write3Ddeconv_startTimes(data_dump_str,s.subjects.(s.id).infusion_resp_rt,s.subjects.(s.id).infusion_resp_rt+1,sprintf('%s_infusion_rating_times',s.id),s.subjects.(s.id).infusion_censor_array',0,s); %Stick (Previously %s_infusion_ratings)
        write3Ddeconv_startTimes(data_dump_str,s.subjects.(s.id).infusionresponse_onset_all_runs,s.subjects.(s.id).infusion_resp_rt+1,sprintf('%s_infusion_rating_times_rt_convolv',s.id),s.subjects.(s.id).infusion_censor_array',0,s); %Stick 
