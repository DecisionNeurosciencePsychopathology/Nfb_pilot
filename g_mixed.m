...
cs = u_t(3);
if cs || inG.noCS
[gx(1),~,~] = sigm((Xt(1)*eta + u_t(2)*epsilon + lambda1),[],beta); %% prediction of feedback ratings as a function of 
...expectancy (Xt) weighted by expectancy sensitivity eta + actual neurofeedback presented (u_t(2)) + bias lambda
    
[gx(2),~,~] = sigm((Xt(1) + lambda2),[],beta); %% prediction of infusion/no infusion expectancy ratings as a function of expectancy (Xt)

% exp_vigor = eta*Xt(1);
else
[gx(1),~,~] = sigm((Xt(2)*eta + u_t(2)*epsilon + lambda1),[],beta);
[gx(2),~,~] = sigm((Xt(2) + lambda2),[],beta);
